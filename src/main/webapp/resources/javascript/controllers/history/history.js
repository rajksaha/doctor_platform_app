app.controller('PrescribeHistoryController', function($scope, $stateParams, $http, $modal, $rootScope, limitToFilter, $location, HistoryService) {
	...
	$scope.bringHistoryDetail = function (){
		...
		HistoryService.getHistoryDetail.query({}, dataString).$promise.then(function (result) {
	...
	$scope.addHistoryToDoctorPref = function (){
		...
		if(validator.validateForm("#historySetting","#lblMsg",null)) {
			if($scope.addByName == false){
				...
				HistoryService.updateHistoryName.query({}, dataString).$promise.then(function (result) {
					if (result && result.success) {
	...
	$scope.addToDoctorPreference = function (historyID){
		...
		HistoryService.setDoctorHistory.query({}, dataString).$promise.then(function (result) {
			if (result && result.success) {
				$scope.bringHistoryDetail();
			} else {
	...
	$scope.bringHistoryOption = function(historydata){
		...
		HistoryService.getHistoryOption.query({}, dataString).$promise.then(function (result) {
			if (result && result.success) {
	...
	$scope.addHistoryOption = function (historydata){
		if(historydata.optionAdder){
			...
			HistoryService.updateHistoryOption.query({}, dataString).$promise.then(function (result) {
				if (result && result.success) {
	...
	$scope.saveHistory = function(){
		...
		angular.forEach(prescribedHistory, function(value, key) {
			if(parseInt(value.patientHistoryID) > 0 && value.historyResult){//Update
				...
				HistoryService.updatePatientHistory.query({}, dataString).$promise.then(function(result) {
					if (result && result.success) {
				...
				HistoryService.createPatientHistory.query({}, dataString).$promise.then(function(result) {
					if (result && result.success) {
		...
		angular.forEach(prescribedHistory, function(value, key) {
			if(parseInt(value.savedHistorysID) > 0 && value.historyResult && value.addToPrescription){
				...
				HistoryService.createPrescritionHistory.query({}, dataString).$promise.then(function(result) {
					if (result && result.success) {
